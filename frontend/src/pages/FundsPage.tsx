  ...
  return (
    <div className="page-stack">
      <div className="page-title-row">
        <div>
          <h2>基金分析</h2>
          <p>基于净值收益、回撤、波动和趋势生成基金研究快照，独立于股票基本面模型。</p>
        </div>
        {selected && <Badge tone={badgeTone(selected.state)}>{selected.nav_date}</Badge>}
      </div>
      ...
      {selected ? (
        <>
          <div className="metric-grid">
            <MetricCard label="当前基金" value={selected.name} hint={selected.symbol} tone="neutral" />
            <MetricCard label="综合评分" value={numberValue(selected.total_score).toFixed(1)} hint={selected.state} tone={badgeTone(selected.state)} />
            <MetricCard label="近 3 月收益" value={percent(selected.return_3m)} hint="净值区间收益" tone={numberValue(selected.return_3m) >= 0 ? 'good' : 'bad'} />
          ...
          <div className="grid-two">
            <Card title="净值趋势" description="最近 180 条基金净值走势。">
              {navLine.length > 0 ? <PriceLine data={navLine} /> : <EmptyState title="暂无净值数据" description="将基金加入观察池并执行每日更新后会显示净值趋势。" />}
            </Card>
            <Card title="风险与趋势" description="基金评分更关注回撤、波动和净值趋势。">
          ...
          <Card title="基金结论" description="结论来自 fund_analysis_snapshot，供人工研究和观察，不代表买卖建议。">
        ...
        <EmptyState title="暂无基金分析" description="先在观察池添加 FUND 类型基金，再执行每日更新。" />
      )}

      <Card title="基金评分列表" description="点击行切换上方分析视图。">
        <table className="data-table clickable-table">
          <thead><tr><th>基金</th><th>状态</th><th>总分</th><th>1 月</th><th>3 月</th><th>6 月</th><th>回撤</th><th>波动</th><th>结论</th></tr></thead>
          <tbody>
            {rows.map((row) => (
