...
def run(job_id: int | None = None) -> None:
    ...
    with connect_db() as conn:
        ...
        try:
            ...
            update_job(conn, job_id, "RUNNING", 74, "计算 ETF 流动性与风险收益")
            etf_risk = build_etf_liquidity_risk_return()

            update_job(conn, job_id, "RUNNING", 74, "计算 ETF 跟踪质量")
            etf_tracking = build_etf_tracking_quality()
            ...
            update_job(
                ...
                (
                    ...
                    f"ETF画像 {etf_profile.get('count', 0)} 个，ETF风险收益 {etf_risk.get('count', 0)} 个，"
